...
ranges_accuracy = np.arange(0.1, 0.2, 0.1)
input_size, output_size = (28*28 if dataset=='MNIST' else 32*32*3), 10
init = ('*' if len(init)==0 else init)
files_pattern = "./weights/{}/{}_{}_{}_nlayers-{}_init-{}_support-{}*".format(dataset, dataset, netsize, architecture, num_layers, init, scaling_factor)  # wildcards for architecture and accuracy
...
for i, acc in enumerate(ranges_accuracy):
    acc_prefix = "{:1.4f}".format(acc)
    extended_acc_prefix = ["{:1.1f}".format(a) for a in np.arange(0.1, 0.2, 0.1)]
    files_ = [files_pattern + 'realloss-{}'.format(ap) + '*.npy' for ap in extended_acc_prefix]
...
for i, acc in enumerate(ranges_accuracy):
    acc_prefix = "{:1.4f}".format(acc)
    extended_acc_prefix = ["{:1.1f}".format(a) for a in np.arange(acc, acc+bins_size, 0.025)]
    files_ = [files_pattern + 'realloss-{}'.format(ap) + '*.npy' for ap in extended_acc_prefix]
    n_files = sum([len(glob.glob(f)) for f in files_])
    print("[logger]: Collecting parameters for {} nets with accuracy {}, with wildcard {}".format(n_files, acc_prefix, files_))
    processed_files, idx_glob, global_files = 0, 0, list(itertools.chain.from_iterable([glob.glob(f) for f in files_]))
