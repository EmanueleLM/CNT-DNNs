...
for i, acc in enumerate(ranges_accuracy):
    ...
    for file_ in glob.glob(files_):
        ...
        CNet = ComplexNetwork(architecture, num_layers, W, input_size, output_size, flatten=True)  # simplify the weights/biases usage
        for l in range(num_layers):
            nodes_strength[l][acc_prefix] = np.concatenate((nodes_strength[l][acc_prefix], CNet.weights[l], CNet.biases[l]))
for l in range(num_layers):
    for i, acc in enumerate(ranges_accuracy):
        acc_prefix = "{:4.4f}".format(acc)
    ...
    plt.ylabel("PDF(S)")
    plt.show()
    fig_name = "{}_{}_nodes-strength_init-{}_support-{}_layer-{}.svg".format(dataset, architecture, (init if init!='*' else 'any'), scaling_factor, l)
    plt.savefig(saved_images_path + fig_name)
    plt.close()
